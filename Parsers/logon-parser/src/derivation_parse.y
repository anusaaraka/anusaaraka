...
sen_grammar1	: LEFT_PAREN {fprintf(d_tree_fp,"%s",$1);} sen_grammar  RIGHT_PAREN {fprintf(d_tree_fp,"%s",$4);}
		...
		| ID {fprintf(d_tree_fp," %s ",$1);} LEX_GRM {fprintf(d_tree_fp," %s ",$3);} ID ID ID 
                | TOKEN {fprintf(d_tree_fp," %s ",$1);} ID {fprintf(d_tree_fp," %s ",$3);} token_grammar1 {count=1;}
                | ID {fprintf(d_tree_fp," %s ",$1);} token_grammar1 {count=0;}
...
token_grammar1 : /* */
               ...
               | TAG   { if(count==1) {word_id=word_id+1;cat=malloc(strlen($1)+1);cat=$1;
