...
#================================ structure correction =======================================
    ...
    if re.findall(r'\(NP \((NNP|NN) (Sir|sir)\)\)\)\) \(. \?\)', line1):
        #		print "rule1"
        line1 = re.sub('\(NP \((NNP|NN) (Sir|sir)\)\)\)\)',
                       r')) (S (NP (\1 \2)))', line1)
...
#===================== category corrections =====================================================
    ...
    if re.findall(r'\((MD|DT) \w+\) \(ADVP \(RB fast\)', line1):
        #print "rule5"
        line1 = re.sub('\((MD|DT) (\w+)\) \(ADVP \(RB fast\)',
                       r'(\1 \2) (NP (NN fast)', line1)
    ...
    if re.findall(r'\(VP \((VBP|VB) (madam|Madam|Sir|sir)', line1):
        #		print "rule 11"
        line1 = re.sub(r'\(VP \((VBP|VB) (madam|Madam|Sir|sir)',
                       r'(NP (NN \2', line1)
...
#		print  m1[1][len(m1[0])-1:], m1[1][:len(m1[0])-1].lower()
        # check singular and countable
        if m1[1][len(m1[0])-1:] == '<n><sg>' and m1[1][:len(m1[0])-1].lower() not in uncountable1.split():
            if len(m1) > 2:
                if m1[2][len(m1[0])-1:].startswith("<vblex>"):					   # check verb form exists
                    line1 = re.sub('\(ROOT \(NP \(NP \(NN (\w+)\)',
                                   r'(ROOT (S (VP (VB \1)', line1)
    print(line1)
