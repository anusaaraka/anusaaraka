...
def print_dbg_info(pid, word, root, cat):
    try:
        ...
        print('throws exception at',
              parserid_dic[sent[int(ids[0])+6:int(ids[1][:-2])+6]])
...
relation_names = ['parg', 'udef', 'def',
                  'poss', 'pron', 'pronoun', 'nominalization']
        ...
        if '^' not in rel_dic[key]:
            ...
            if root[0] not in relation_names:
                if '/' in root[0]:  # He [ladles] it with a large wooden spoon.
                    lst = root[0].split('/')
                    root[1] = lst[1]
                dic[int(parserid_dic[key])] = parser_word + \
                    ':' + root[0] + ':' + root[1]
        else:
...
for key in sorted(dic):
    ...
    if len(val) > 2:
        ...
        print_dbg_info(key, val[0], val[1], '-')
