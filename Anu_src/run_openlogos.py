...
lupwa_prep = ['again', 'later', 'here', 'there', 'somewhere', 'anywhere', 'everywhere', 'now', 'outside',
              'longer', 'alone', 'next', 'upstairs', 'downstairs', 'upwards', 'downwards', 'above', 'down', 'up', 'where']
# NOTE: kriyA-lupwa_prep_saMbanXI  is now changed to kriyA-aXikaraNavAcI_avyaya
cat = {'1': 'noun', '2': 'verb', '3': 'adverb', '4': 'adjective', '5': 'pronoun', '6': 'adverb', '8': 'Clausal Constructions', '11': 'preposition', '12': 'verb',
       '13': 'preposition', '14': 'determiner', '15': 'determiner', '16': 'Arithmates', '17': 'Negatives', '18': 'Relatives/Interrogatives', '19': 'conjunction', '20': 'punctuation'}
kAlavAcI = ['today', 'night', 'yesterday', 'tomorrow', 'week', 'month', 'year', 'sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'january',
            'february', 'march', 'april', 'may', 'june', 'july', 'august', 'september', 'october', 'november', 'december', 'season', 'evening', 'day', 'hour', 'morning']
kri = ''
upa = ''
prep_dict_word = {}
prep_dict_id = {}
prep_dict_id_semwrk = {}
prep_dict_word_semwrk = {}
switch68_pos = []
...
for i in range(1, len(SCON_FOR_tran4)-1):
...
for i in range(len(RES_SWRK_TAB)):
    group_str = ''
    if RES_SWRK_TAB[i][34] == '1' and RES_SWRK_TAB[i][35] == 'bos':
        ol_res_id_map.write(
            "(ol_res_id-word_id-word\tP%s\t0\tbos)\n" % (RES_SWRK_TAB[i][0]))
    elif RES_SWRK_TAB[i][35] in punc_list:
        ol_res_id_map.write(
            "(ol_res_id-word_id-word\tP%s\t-\tpunctuation_mark)\n" % (RES_SWRK_TAB[i][0]))
        ...
        if len(wrd_lst) == 1 and int(RES_SWRK_TAB[i][34]) > 1:
            ...
            for j in range(len(wrd_lst)):
                ...
                if j == len(wrd_lst)-1:
                    ...
                    ol_res_id_map.write(
                        "(ol_res_id-word_id-word\tP%s\t%s.1\t%s)\n" % (RES_SWRK_TAB[i][0], wrdid, grp_str))
                    ol_res_id_map.write(
                        "(current_id-group_members\t%s.1\t" % wrdid)
                    grp_num_lst = []
                for k in range(int(RES_SWRK_TAB[i][34])):
                    if wrdid-k not in grp_num_lst:
                        grp_num_lst.append(wrdid-k)
                        grp_num_lst.sort()
                        grp_num_str = ''
                for l in range(len(grp_num_lst)):
                    grp_num_str = grp_num_str + ' ' + str(grp_num_lst[l])
                    grp_num_str = grp_num_str.strip()
            ol_res_id_map.write("%s)\n" % grp_num_str)
            ol_res_id_map.write(
                ";The relation name 'current_id-group_members' is used temporarely and should be replaced by 'head_id-group_members'.\n")
        wrdid += 1
    elif int(RES_SWRK_TAB[i][34]) == 1:
        ol_res_id_map.write("(ol_res_id-word_id-word\tP%s\t%s\t%s)\n" %
                            (RES_SWRK_TAB[i][0], wrdid, RES_SWRK_TAB[i][35].lower()))
...
goru_counter = 0
for i in range(len(OP_TAR_ARR_TRAN3)):
    if 'SWORKO' in OP_TAR_ARR_TRAN3[i]:
        if OP_TAR_ARR_TRAN3[i][0].endswith(')'):
            OP_TAR_ARR_TRAN3[i].insert(0, 'hpg')
#--------------------------------
for i in range(len(semwrk_prep)):
    for k in range(len(switch68_pos)):
        for j in range(7, len(semwrk_prep[i])):
            if int(semwrk_prep[i][j]) > int(switch68_pos[k]):
                semwrk_prep[i][j] = str(int(semwrk_prep[i][j])-1)

for i in range(len(semwrk_verb)):
    for k in range(len(switch68_pos)):
        for j in range(7, len(semwrk_verb[i])):
...
tmp_prep = ''
tmp_head = ''
for j in range(0, len(semwrk_prep)):
    for i in range(4, len(semwrk_prep[j]), 4):
        if semwrk_prep[j][i] == '13' and i+7 <= len(semwrk_prep[j]):
            for hr in range(len(RES_SWRK_TAB)):
            ...
            if semwrk_prep[j][i+7] in list(prep_dict_id_semwrk.keys()) and prep_dict_id_semwrk[semwrk_prep[j][i+7]] != semwrk_prep[j][i+3]:
                prep_dict_id_semwrk[semwrk_prep[j][i+7]+'_part_' +
                                    repr(prep_counter)] = semwrk_prep[j][i+3]
                prep_dict_word_semwrk[tmp_head +
                                      '_part_'+repr(prep_counter)] = tmp_prep
                prep_counter += 1
            else:
...
for i in range(len(OP_TAR_ARR_TRAN3)):
    if len(OP_TAR_ARR_TRAN3[i]) > 0:
        ...
        # ex. The metric system is designed around a unit of distance, the meter, a unit of mass, the kilogram, and a time unit, the second.
        if 'SCONHF' in OP_TAR_ARR_TRAN3[i] and "SWORKO" in OP_TAR_ARR_TRAN3[i-5]:
...
for i in range(len(SWRK_TAB_TRN4_0)):
    c = ''
    c = c + SWRK_TAB_TRN4_1[i]
    for j in range(len(SWRK_TAB_TRN4_0[i])):
...
id_comp_wrd_space = []
id_comp_wrd_un = []
samAsa_wrds_with_underscore = ''
samAsa_wrds_with_space = ''
samAsa = ''
Verb_Particle_list = []
...
aux_verb_lst = ['do', 'did', 'does', 'is', 'are', 'am', 'was', 'were', 'will',
                'shall', 'should', 'can', 'could', 'would', 'has', 'have', 'may', 'May']
aux_verb_lst_small = ['did', 'does', 'is', 'are', 'am', 'was', 'were', 'will',
                      'shall', 'should', 'can', 'could', 'would', 'has', 'have', 'may', 'May']
...
for i in range(len(SWRK_TAB_TRN4)):
...
color_list = ["blue", "gray", "yellow", "orange", "red", "violet", "maize",
              "green", "umber", "thistle", "mulberry", "pink", "white", "black"]
...
propN_lst = ['John', 'Mary', 'Rama', 'Sita', 'Seeta', 'Gita', 'Roja', 'Sriram', 'Dipti', 'Amba', 'Lakshmi', 'Laxmi', 'Sonal', 'Sonia',
             'Gandhi', 'India', 'Delhi', 'Aperna', 'Lipi', 'Meena', 'Shital', 'Vineet', 'Sukhada', 'Veena', 'Aditi', 'Shibani', 'Manju', 'Grace', 'UK']
...
matrix = [[]]
for j in range(1, len(OP_TAR_ARR_TRAN3)):
...
for i in OP_TAR_ARR_TRAN3:
    ...
    if 'BOS' in B and 'OPADRO' in S and 'SCONPO' in O:
        for j in O:
            if j == '2':
                rel_fp.write(
                    "(prep_id-relation-parser_ids - wall_conjunction P2)\n")
# Ex. However the user needs some training to use the software effectively.

# Within pada relations:
id_word_tuple = list(id_word.items())
kriyA_id = ''
upasarga_id = ''
for each in range(len(Verb_Particle_list)):
...
y = ''
prep_counter = 2
for i in range(0, len(tran3_components)):
    if tran3[i][4] == '2':
        for x in tran3_components[i]:
            if x != 'SCONPO':
                y = x
                if int(x) <= len(sent_wrd_list):
                    for j in range(len(RES_SWRK_TAB)):
                        if RES_SWRK_TAB[j][0] == y:
                            # and id_word[RES_SWRK_TAB[j][0]] not in lupwa_prep:
                            if (RES_SWRK_TAB[j][2] == '3' or RES_SWRK_TAB[j][2] == '6') and id_word[RES_SWRK_TAB[j][0]] not in kAlavAcI:
        ...
        for j in range(len(RES_SWRK_TAB)):
            if RES_SWRK_TAB[j][0] == matrix[0][i][-1]:
                if (RES_SWRK_TAB[j][2] == '3' or RES_SWRK_TAB[j][2] == '6'):
                    ...
                    if matrix[0][i][8] in list(prep_dict_word.keys()):
                        prep_dict_id[matrix[0][i][7]+'_part_' +
                                     repr(prep_counter)] = RES_SWRK_TAB[j][0]
                        prep_dict_word[matrix[0][i][8]+'_part_' +
                                       repr(prep_counter)] = RES_SWRK_TAB[j][35]
                        prep_counter += 1
                    else:
...
#--------------------------------------------------------------------------------------------------------------
conjunction_list = ['so', 'that', 'because',
                    'where', 'when', 'but', 'without', 'after']
...
aux_verb_lst = ['do', 'did', 'does', 'is', 'are', 'am', 'was', 'were', 'will',
                'shall', 'should', 'can', 'could', 'would', 'has', 'have', 'may', 'May']
prep_lst = ['aboard', 'about', 'above', 'across', 'after', 'against', 'along', 'alongside', 'amid', 'amidst', 'among', 'amongst', 'around', 'as', 'aside', 'astride', 'at', 'athwart', 'atop', 'barring', 'before', 'behind', 'below', 'beneath', 'beside', 'besides', 'between', 'beyond', 'by', 'circa', 'concerning', 'despite', 'down', 'during', 'except', 'excluding', 'failing', 'following', 'for', 'from', 'in',
            'including', 'inside', 'into', 'like', 'mid', 'minus', 'near', 'next', 'notwithstanding', 'of', 'off', 'on', 'onto', 'opposite', 'out', 'outside', 'over', 'pace', 'past', 'per', 'plus', 'qua', 'regarding', 'round', 'since', 'than', 'through', 'throughout', 'till', 'times', 'to', 'toward', 'towards', 'under', 'underneath', 'unlike', 'until', 'up', 'upon', 'versus', 'via', 'with', 'within', 'without', 'worth']
...
got_kqxnwa_rel = ''
for each in range(len(SWRK_TAB_TRN4_V1)):
...
# Ex. The boy you called yesterday has arrived. The snake the mongoose attacked hissed loudly.
        if i > 2:
            if trn3[i][0] == '20' and trn3[i][1] == '103' and trn3[i][2] == '19' and trn3[i+1][0] == '1' and trn3[i][4] != 'that':
                rel_fp.write(
                    "(prep_id-relation-parser_ids - subject-conjunction  P%s\tP10000)\n" % sconId_resId[trn3[i+1][3]])
                pada_fp.write("(pada_info (group_head_id P10000) (group_cat PP) (group_ids P10000) (vibakthi 0) (gender 0) (number 0) (case 0) (person 0) (H_tam 0) (tam_source 0) (preceeding_part_of_verb 0) (preposition 0) (Hin_position 0))\n")
                original_wrd_fp.write("(parserid-wordid P10000 10000)\n")
...
# Ex. The book I was reading is wonderful.
    ...
    # 'not in lupwa_prep' condition is for 'Here is a big surprise for you.'
    if tran3[i][8] in copula and not(tran3[i][4] == '12') and not(tran3[i-1][4] == '2' or tran3[i-1][4] == '12') and tran3[1][8] not in lupwa_prep and tran3[i+1][8] != 'a' and tran3[i-1][9] != 'SWITCH68' and tran3[1][9] != 'SWITCH68':
        prep_obj = list(prep_dict_word_semwrk.keys())
        # She sold the house even though it was against his wishes. I thought it is right to resign.
        if tran3[i+1][8] not in prep_obj and tran3[i+1][4] != '2':
            rel_fp.write("(prep_id-relation-parser_ids - subject-subject_samAnAXikaraNa  P%s\tP%s)\n" %
                         (sconId_resId[tran3[i-1][7]], sconId_resId[tran3[i+1][7]]))

    # 'not in lupwa_prep' condition is for 'Here is a big surprise for you.'
    if tran3[i][8] in copula and not(tran3[i][4] == '12') and not(tran3[i-1][4] == '2' or tran3[i-1][4] == '12') and tran3[1][8] not in lupwa_prep and tran3[i+1][8] != 'a' and tran3[i-1][8] != 'how' and tran3[i-1][9] != 'SWITCH68':
        rel_fp.write("(prep_id-relation-parser_ids - kriyA-subject  P%s\tP%s)\n" %
                     (sconId_resId[tran3[i][7]], sconId_resId[tran3[i-1][7]]))
# Ex.	THIS IS a sample SENTENCE for Anusaaraka.

    # 'not in lupwa_prep' condition is for 'Here is a big surprise for you.'
    if tran3[i][8] in copula and not (tran3[i][4] == '12') and (tran3[i-1][4] == '2' or tran3[i-1][4] == '12') and tran3[1][8] not in lupwa_prep and tran3[i-2][9] != 'SWITCH68' and tran3[i+1][9] != 'SWITCH68':
        rel_fp.write("(prep_id-relation-parser_ids - subject-subject_samAnAXikaraNa  P%s\tP%s)\n" %
                     (sconId_resId[tran3[i-2][7]], sconId_resId[tran3[i+1][7]]))
# Ex.	THIS IS a sample SENTENCE for Anusaaraka.
...
# Ex. I have started working.
    ...
    # the condition tran3[i+2][6] != '38 is for 'I want to go there.'
    elif tran3[i][4] == '2' and tran3[i+1][4] == '2' and tran3[i+1][6] != '38' and (tran3[i+2][4] == '1') and (tran3[i+1][9] != 'SWITCH68' and tran3[i][9] != 'SWITCH68' and tran3[i-1][9] != 'SWITCH68'):
        rel_fp.write("(prep_id-relation-parser_ids - kriyA-subject  P%s\tP%s)\n" %
                     (sconId_resId[tran3[i+1][7]], sconId_resId[tran3[i-1][7]]))
        rel_fp.write("(prep_id-relation-parser_ids - kriyA-object  P%s\tP%s)\n" %
                     (sconId_resId[tran3[i+1][7]], sconId_resId[tran3[i+2][7]]))
...
# Ex.    A fat ugly boy had to eat too many FRUITS to LOSE his weight. +To stop kriyA-subject rel bitween
    ...
    # the condition tran3[i+2][6] != '38 is for 'I want to go there.'
    elif (i == 1 and tran3[i][4] == '1' and (tran3[i+1][4] == '2' or tran3[i+1][4] == '12') and tran3[i+2][4] == '2' and tran3[i+2][6] != '38') and tran3[i][9] != 'SWITCH68' and tran3[i+2][9] != 'SWITCH68':
        rel_fp.write("(prep_id-relation-parser_ids - kriyA-subject  P%s\tP%s)\n" %
...
#------------------------------------------------------------------------------------------------------------------------
        ...
        for k in range(1, (len(tran3)-i)):
            if (tran3[i+k][4] == '2' or tran3[i+k][4] == '20'):
                break
            if (tran3[i+k][4] == '1'):
                # ex. 4 > & < conditions: "Imagine that a magician waves his wand and changes a bowl of dirt into a bowl of leuttce." the last condition is added for the sentence: "A pool always needs to have its water circulated through a filter for several hours a day in order to keep it clean. and "The factory typically produces 500 chairs a week."
                if tran3[i+k][8] in list(prep_dict_word_semwrk.keys()) and tran3[i][9] != 'SWITCH68' and int(word_id[prep_dict_word_semwrk[tran3[i+k][8]]]) > int(sconId_resId[tran3[i][7]]) and int(word_id[prep_dict_word_semwrk[tran3[i+k][8]]]) < int(sconId_resId[tran3[i+k][7]]) and prep_dict_word_semwrk[tran3[i+k][8]] != 'a':
                    rel_fp.write("(prep_id-relation-parser_ids P%s kriyA-%s_saMbanXI  P%s\tP%s)\n" % (sconId_resId[prep_dict_id_semwrk[tran3[i+k][7]]], return_prep(
                        word_id[prep_dict_word_semwrk[tran3[i+k][8]]]), sconId_resId[tran3[i][7]], sconId_resId[tran3[i+k][7]]))
                    ...
                    got_prep_rel = list(prep_dict_word_semwrk.keys())
                else:
                    for hr in range(len(RES_SWRK_TAB)):
                        if tran3[i+k][7] == RES_SWRK_TAB[hr][0]:
                            if RES_SWRK_TAB[hr][2] != '3' and RES_SWRK_TAB[hr][2] != '6':
                                # and tran3[i+k][8] not in prep_lst: #ex for 2nd and condition 'He left in the morning.' These shoes that I bought will look nice with that hat. To the people of India he is the symbol of mature wisdom.
                                if tran3[i+k][8] not in copula and tran3[i+k][6] != '96' and tran3[i+k][6] != '90':
                                    gor_counter += 1
                                    gor_kriyA.append(tran3[i][7])
                                    gor_object.append(tran3[i+k][7])
                                    if gor_counter == 2:
                                        got_obj = tran3[i][4]
    # the last part in this con is to stop object relation for prepositional objects. ex. "Ayodhya during ancient times was known as Kaushaldesa."
    if gor_counter == 1 and gor_kriyA[0] in list(id_word.keys()) and id_word[gor_kriyA[0]] not in copula and lupwa_sub_kri == '' and gor_object[0] not in list(pobj_prep.keys()):
        rel_fp.write("(prep_id-relation-parser_ids - kriyA-object  P%s\tP%s)\n" %
...
#	    print "yteery - kriyA-object  P%s\tP%s)\n" % (sconId_resId[gor_kriyA[0]],sconId_resId[gor_object[0]])
    ...
    if gor_counter == 2 and sconId_resId[gor_kriyA[0]] != '0' and id_word[sconId_resId[gor_kriyA[0]]] not in copula and id_word[sconId_resId[gor_object[1]]] not in prep_lst:
        ...
        for hr in range(len(RES_SWRK_TAB)):
            if tran3[i][7] == RES_SWRK_TAB[hr][0]:
                if RES_SWRK_TAB[hr][2] != '3' and RES_SWRK_TAB[hr][2] != '6' and tran3[i+1][9] != 'SWITCH68':
                    if tran3[i+1][7] in list(prep_dict_id_semwrk.keys()) and tran3[i+1][8] in list(prep_dict_word_semwrk.keys()):
                        rel_fp.write("(prep_id-relation-parser_ids P%s viSeRya-%s_saMbanXI  P%s\tP%s)\n" % (
...
#-----------------------------------------------------------------------------------------------------------------
    ...
    # got_kqxnwa_rel = '' condition is to stop in "She rose from the table to welcome me."
    elif tran3[i][4] == '2' and (tran3[i][6] == '28' or tran3[i][6] == '38') and tran3[i-1][4] == '1' and got_kqxnwa_rel == '' and tran3[i][9] != 'SWITCH68' and tran3[i-1][9] != 'SWITCH68' and got_kqxnwa_rel != tran3[i][7]:
...
# Ex. I wonder how much you swim.
    ...
    elif len(tran3) > 4 and tran3[1][7] in list(prep_dict_id_semwrk.keys()) and tran3[2][4] == '12' and int(prep_dict_id_semwrk[tran3[1][7]]) < int(tran3[1][7]):
        rel_fp.write("(prep_id-relation-parser_ids P%s kriyA-%s_saMbanXI  P%s\tP%s)\n" % (prep_dict_id_semwrk[tran3[1][7]], return_prep(
            prep_dict_id_semwrk[tran3[1][7]]), sconId_resId[tran3[2][7]], sconId_resId[tran3[1][7]]))
...
for j in range(0, len(semwrk_verb)):
    if len(semwrk_verb) > 0 and semwrk_verb[j][4] == '2':
        id_key = semwrk_verb[j][7]
        id_scon_keys = list(sconId_resId.values())
        id_word_keys = list(id_word.keys())
        ...
        # the condition semwrk_verb[j][6] != '38 is for 'I want to go there.'
        if semwrk_verb[j][10] == '91' and semwrk_verb[j][6] != '38':
            rel_fp.write("(prep_id-relation-parser_ids - kriyA-subject  P%s\tP%s)\n" %
                         (sconId_resId[semwrk_verb[j][7]], sconId_resId[semwrk_verb[j][11]]))
        if len(semwrk_verb[j-1]) > 15:
            # the condition semwrk_verb[j][6] != '38 is for 'I want to go there.'
            if semwrk_verb[j-1][10] == '94' and semwrk_verb[j-1][14] == '91' and semwrk_verb[j-1][6] != '38':
                rel_fp.write("(prep_id-relation-parser_ids - kriyA-subject  P%s\tP%s)\n" %
                             (sconId_resId[semwrk_verb[j-1][7]], sconId_resId[semwrk_verb[j-1][15]]))
# Ex.
        # and semwrk_verb[j-1][14] == '91' and semwrk_verb[j-1][6]!='38': #the condition semwrk_verb[j][6] != '38 is for 'I want to go there.'
        if semwrk_verb[j-1][6] in passive_verb_form and semwrk_verb[j-1][10] == '94':
            rel_fp.write("(prep_id-relation-parser_ids - kriyA-subject  P%s\tP%s)\n" %
                         (sconId_resId[semwrk_verb[j-1][7]], sconId_resId[semwrk_verb[j-1][11]]))
# Ex.  Are John or I invited?

        for k in range(8, len(semwrk_verb[j]), 4):
            if semwrk_verb[j][k] == '1' and not(semwrk_verb[j][k+3] in list(prep_dict_id_semwrk.keys())) and (semwrk_verb[j][k+2] != '91'):
    ...
    if len(semwrk_verb[j]) > 15 and len(gor_kriyA) > 0:
        ...
        if id_key in id_word_keys and lupwa_sub_kri == '':
            # the second condition is to stop in passive sentences ex. 'Ravana was killed by Rama' #ex for 'semwrk_verb[j][14] != '96'" condition 'He left in the morning.' These shoes that I bought will look nice with that hat.
            if gor_counter_semwrk == 1 and sconId_resId[gor_kriyA[0]] != '0' and semwrk_verb[j][6] not in passive_verb_form and semwrk_verb[j][6] != '61' and id_word[sconId_resId[gor_kriyA[0]]] not in copula and semwrk_verb[j][14] != '96' and semwrk_verb[j][14] != '90' and semwrk_verb[j][13] != '401':
                rel_fp.write("(prep_id-relation-parser_ids - kriyA-object  P%s\tP%s)\n" %
                             (sconId_resId[gor_kriyA[0]], sconId_resId[gor_object[0]]))
...
# ex. These shoes that I bought will look nice with that hat.
    if len(gor_kriyA) > 0 and len(gor_object) == 1:
        ...
        if id_key in id_word_keys and id_obj_key in id_word_keys and sconId_resId[gor_kriyA[0]] != '0':
            # I have been grading these stupid exams all day. I woke up early this morning.
            if gor_counter_semwrk == 2 and id_word[sconId_resId[gor_kriyA[0]]] not in copula and id_word[sconId_resId[gor_object[1]]] not in kAlavAcI:
                rel_fp.write("(prep_id-relation-parser_ids - kriyA-object_1  P%s\tP%s)\n" %
                             (sconId_resId[gor_kriyA[0]], sconId_resId[gor_object[0]]))
                rel_fp.write("(prep_id-relation-parser_ids - kriyA-object_2  P%s\tP%s)\n" %
                             (sconId_resId[gor_kriyA[1]], sconId_resId[gor_object[1]]))
...
kriyA_mUla_lst = ['get', 'got', 'gets',
                  'getting', 'make', 'made', 'makes', 'making']
for i in range(len(tran3)):
...
# Ex. A FAT UGLY BOY had to eat too many fruits to lose his weight .
                ...
                if sconId_resId[tran1_components[i][j]] != '0' and RES_SWRK_TAB[k][2] == '6' and RES_SWRK_TAB[k+1][2] == '1' and (id_form_dic[sconId_resId[tran1_components[i][j]]] != '4' and id_form_dic[sconId_resId[tran1_components[i][j]]] != '7'):
    ...
    if tran1[i][0] == '1' and tran1[i][5] != 'SWITCH68':
        ...
        for j in range(1, len(tran1_components[i])):
            if tran1_components[i][j] != '0' and int(tran1_components[i][j]) < int(RaRTI_viSeRya):
                ...
                if sconId_resId[RaRTI_vNa] != '0' and (id_form_dic[sconId_resId[RaRTI_vNa]] == '4' or id_form_dic[sconId_resId[RaRTI_vNa]] == '7') and id_word[sconId_resId[RaRTI_vNa]] not in RaRTI_viSeRaNa and(RESid_cat_dic[sconId_resId[RaRTI_vNa]] != '14' or RESid_cat_dic[sconId_resId[RaRTI_vNa]] != '15'):
                    rel_fp.write("(prep_id-relation-parser_ids - viSeRya-RaRTI_viSeRaNa  P%s\tP%s)\n" %
...
# Ex. Mary's shoes are red. These are students' books.
    ...
    if sconId_resId[tran1[i][0]] == '1':
        # The boy you called yesterday has arrived. The snake the mongoose attacked hissed loudly.
        if sconId_resId[tran1[i][3]] != '0' and (id_form_dic[sconId_resId[tran1[i][3]]] == '4' or id_form_dic[sconId_resId[tran1[i][3]]] == '7') and RESid_cat_dic[sconId_resId[tran1[i][3]]] != '5' and RESid_cat_dic[sconId_resId[tran1[i+1][3]]] != '2' and tran1[i][5] != 'SWITCH68' and tran1[i+1][5] != 'SWITCH68':
            rel_fp.write("(prep_id-relation-parser_ids - viSeRya-RaRTI_viSeRaNa  P%s\tP%s)\n" %
                         (sconId_resId[tran1[i+1][3]], sconId_resId[tran1[i][3]]))
...
for i in range(len(tran3)):
    for j in range(1, len(tran3_components[i])):
...
for i in range(len(tran2)):
    for j in range(len(tran2)-1):
...
preplist = list(prep_dict_id_semwrk.values())
...
for i in range(len(SWRK_TAB_TRN4)):
    if SWRK_TAB_TRN4[i][1] == '2' and SWRK_TAB_TRN4[i][3] == '20':
        rel_fp.write(
            "(prep_id-relation-parser_ids - AjFArWaka_kriyA  P%s)\n" % SWRK_TAB_TRN4[i][4])
        i += 1
# Ex. TELL me an imperative sentence.
    ...
    for j in range(0, len(semwrk_verb)):
        for k in range(8, len(semwrk_verb[j]), 4):
            if semwrk_verb[j][k] == '1' and not(semwrk_verb[j][k+3] in list(prep_dict_id_semwrk.keys())) and (semwrk_verb[j][k+2] != '91'):
                ...
                gor_kriyA.append(semwrk_verb[j][7])
                gor_object.append(semwrk_verb[j][k+3])
                prep_key = list(prep_dict_id_semwrk.keys())
                if prep_key:
                    p_key = prep_key[0]
