...
sub Process
{
   ...
   for $i (0..$#words)
   {
	  ...
	  elsif($words[$i] =~ /<\/[A-Za-z]+>$/) {
		# print "word is $words[$i]\n";
		
		# Added by Amba to take care of empty paragraphs.
		if($sentence eq "") { 
		print STDERR "Paragraph is empty check for <p></p> pattern in your file.\n";
		$sentence = "EMPTY PARAGRAPH";
		}
		else {
		# Eliminate any leading whitespace
		$sentence = substr($sentence, 1);
