...
import sys
# generates file which stores info about the original and hyphenated words
wrdSub = open(sys.argv[2], "w")
...
for i in open(sys.argv[1]):
    if '_' in i:
        ...
        wrdSub.write(
            "(para_id-sent_id-word_id-original_word-hyphenated_word\t%s\t%s\t%s)\n" % (piw, origWrd, rest))
    else:
        wrdSubSents.write(i)
