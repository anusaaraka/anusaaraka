...
; )
 ...
 (deffunction print_padasutra_row(?p_id ?s_id ?w_id ?chnk_fr_htm ?l_punc ?r_punc ?padasuthra)
 ...
 (deffunction print_dictionary_row(?p_id ?s_id ?w_id ?chnk_fr_htm ?def_mng ?other_mngs)
 (printout fp "<tr class=\"row4\">" crlf)
 (if (= ?w_id 1) then (printout fp "<td class=\"number\">"?p_id"."?s_id".D</td>"))
 (if (eq ?def_mng -) then
 (printout fp "<td class=\""?chnk_fr_htm"\"> " (wx_utf8 ?def_mng) " </td>" crlf "</tr>" crlf)
 else
 (printout fp "<td class=\""?chnk_fr_htm"\" title=\""(wx_utf8 ?other_mngs)"\"> " (wx_utf8 ?def_mng) " </td>" crlf "</tr>" crlf))
 )
 ...
 (sen_type-id-phrase ?sen_type ?id ?phrase)
 (id-default_mng-iit_bombay_shabdanjali_dic_mng  ?id  ?def_mng ?other_mngs)
 (Eng_sen ?Eng_sen)
 =>
 (retract ?f)
 (if (eq ?r_punc "NONE") then (bind ?r_punc ""))
 ...
 (print_padasutra_row  1 1 1 ?chnk_fr_htm ?l_punc ?r_punc ?padasuthra)
 (print_dictionary_row  1 1 1 ?chnk_fr_htm ?def_mng ?other_mngs)
 (print_root_row  1 1 1 ?chnk_fr_htm ?l_punc ?r_punc ?root)
 ...
 (print_dictionary_row  ?p_id ?s_id ?id ?chnk_fr_htm ?def_mng ?other_mngs)
 (print_root_row  ?p_id ?s_id ?id ?chnk_fr_htm ?l_punc ?r_punc ?root)
 ...
 (print_dictionary_row  ?p_id ?s_id ?id ?chnk_fr_htm ?def_mng ?other_mngs)
 (print_root_row  ?p_id ?s_id ?pp_id ?chnk_fr_htm ?l_punc ?r_punc ?pp_root)
 ...
 (print_root_row  ?p_id ?s_id ?id ?chnk_fr_htm ?l_punc ?r_punc ?word)
