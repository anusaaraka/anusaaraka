...
; (bind ?kri1 (string-to-field ?kri1))
 ...
 ?f0<-(using-parser-ids to-infinitive  ?to ?inf)
 ...
 ?f<-(using-parser-ids viSeRya-jo_samAnAXikaraNa ?vi ?jo)
 ...
 (declare (salience 1000))
 ?f<-(using-parser-ids kriyA-subject  ?kriyA 10001)
 ...
 (declare (salience 1000))
 ?f<-(using-parser-ids kriyA-conjunction  ?sub 10000)
 (not (has_been_included_in_paxa 10000))
 =>
 ;       (retract ?f)
        (assert (has_been_included_in_paxa 10000))
 ...
 (declare (salience 1000))
 (root-verbchunk-tam-chunkids ? ? ? $?ids ?kri)
 ?f1<-(using-parser-ids  ?rel  ?kri ?PP)
 ...
 (defrule PP_pada2
 (declare (salience 1000))
 ?f2<-(using-parser-ids  viSeRya-RaRTI_viSeRaNa  ?viSeRya ?viSeRaNa)
 ...
 (defrule PP_pada3
 (declare (salience 1001))
 ?f1<-(using-parser-ids  ?rel ?id ?id1)
 ...
 ?f0<-(using-parser-ids  ?rel ?viSeRya1 ?viSeRaNa)
 ...
 ?f0<-(using-parser-ids  viSeRaNa-viSeRaka|viSeRya-viSeRaka ?viSeRaNa ?viSeRaka)
 ...
 ?f0<-(using-parser-ids ?rel ?kri ?id) 
 ...
 (declare (salience 980))
 ?f0<-(using-parser-ids saMjFA-kqxanwa ?id ?kri)
 (not (root-verbchunk-tam-chunkids ? ? ? $?ids ?kri))
 ?f1<-(pada_info (group_head_id ?kri)(group_ids  $?grp_ids)(group_cat ?gtype)(pada_head ?PH))
 ?f2<-(pada_info (group_head_id ?id)(group_ids  $?grp_ids1)(pada_head ?ph))
 ?f3<-(id-current_id  ?kri ?current_id)
 ...
 ?f0<-(using-parser-ids  viSeRya-RaRTI_viSeRaNa ?id ?id1)
; (not (root-verbchunk-tam-chunkids ? ? ? $?ids ?kri))
 ?f1<-(pada_info (group_head_id ?kri)(group_ids  $?grp_ids)(group_cat ?gtype)(pada_head ?PH))
 ...
 ?f0<-(using-parser-ids to-infinitive ?to ?kri)
 (not (root-verbchunk-tam-chunkids ? ? ? $?ids ?kri))
 ?f1<-(id-current_id  ?kri ?current_id)
 (current_id-group_members ?current_id $?grp_mems)
 (test (member$ ?kri $?grp_mems))
 ...
 ?f1<-(using-parser-ids kriyA-object ?id1 ?id)
 ...
 ?f0<-(non_finte_verb_unordered_list ?kri $?grp_mems)
 ?f1<-(using-parser-ids saMjFA-kqxanwa ?id1 ?kri)
 ...
 ?f0<-(non_finte_verb_unordered_list ?kri $?grp_mems)
 ?f1<-(using-parser-ids viSeRya-RaRTI_viSeRaNa ?id1 ?id)
 (test (and (member$ ?id $?grp_mems)(member$ ?id1 $?grp_mems)))
 =>
 (retract ?f0 ?f1)
 (bind $?grp_mems (delete-member$ $?grp_mems ?id))
 ...
 (declare (salience 960))
 ?f0<-(non_finte_verb_unordered_list ?kri $?grp_mems)
 ?f1<-(using-parser-ids  ?rel ?kri ?id)
  (test (neq (str-index "-" ?rel)  FALSE))
  (test (neq (str-index "_" ?rel)  FALSE))
...
;  ?f2<-(id-current_id  ?conj ?current_id)
 ...
 ?f0<-(using-parser-ids ?rel ?id ?id1)
 ...
 (test (and (member$  ?id $?grp_mems)(member$  ?head $?grp_mems)))
  =>
 (retract ?f0)
 (assert (using-parser-ids ?rel ?current_id ?id1)))
 ;-------------------------------------------------------------------------------------------------------------
 (defrule map_right_relations
 ?f0<-(using-parser-ids ?rel ?id1 ?id)
 (root-verbchunk-tam-chunkids ? ? ? $?ids)
 (test (not (member$ ?id $?ids)))
 (id-current_id  ?head ?current_id)
 (current_id-group_members ?current_id $?grp_mems)
 ...
 (test (and (member$  ?id $?grp_mems)(member$  ?head $?grp_mems)))
  =>
 (retract ?f0)
 (assert (using-parser-ids ?rel ?id1 ?current_id)))
 ;-------------------------------------------------------------------------------------------------------------
 (defrule delete_relations
 ?f0<-(using-parser-ids ?rel ?id ?id)
 =>
 (retract ?f0)
 )
 ;-------------------------------------------------------------------------------------------------------------
