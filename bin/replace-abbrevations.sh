...
sed 's/@PUNCT/PUNCT/g'  $1 |  sed  's/{number:/{/g' | sed 's/{vib:/{/g' | sed 's/{tam:है/{/g' | sed 's/{tam:/{/g' | sed 's/{}//g' | sed 's/&quot;/\"/g'|sed 's/\&amp;/&/g'| sed 's/PUNCT-[@]*DotDotDot/.../g' | sed 's/PUNCT-[@]*DotDoubleQuote/."/g'| sed  's/PUNCT-[@]*Comma/,/g' | sed  's/PUNCT-[@]*Dot/\./g'|sed  's/PUNCT-[@]*Semicolon/;/g'|sed  's/PUNCT-[@]*Colon/:/g'|sed  "s/PUNCT-[@]*SingleQuote/'/g" |sed  's/PUNCT-[@]*DoubleQuote/"/g'| sed 's/PUNCT-[@]*QuestionMark/?/g' |sed 's/PUNCT-[@]*Exclamation/!/g'| sed 's/PUNCT-[@]*OpenParen/(/g' |sed 's/PUNCT-[@]*ClosedParen/)/g' | sed 's/PUNCT-[@]*OpenClosedParen/()/g' | sed 's/PUNCT-[@]*ClosedParenDot/)\./g' | sed 's/PUNCT-[@]*OpenClosedParenComma/(),/g'| sed "s/PUNCT-[@]*SingleQuoteComma/',/g" | sed "s/PUNCT-[@]*TwoSingleQuotes/''/g"  |  sed 's/PUNCT-[@]*Hyphen/-/g' | sed 's/@\([\+=#β\$]\)/\1/g' | sed 's/\([0-9]\)/\1/g' | sed 's/ SYMBOL-SLASH /\//g' | sed 's/PUNCT-[@]*LeftCurlyBrace/{/g' | sed 's/PUNCT-[@]*RightCurlyBrace/}/g' | sed 's/PUNCT-[@]*LeftSquareBracket/[/g' | sed 's/PUNCT-[@]*RightSquareBracket/]/g' | sed 's/\.[ ]*$/ \./g' > $2
