...
# sh run_transliteration.sh $MYPATH/tmp $1
 ...
 ./rm_tags.out < $MYPATH/tmp/$1_tmp/new_text.txt > $MYPATH/tmp/$1_tmp/one_sentence_per_line.txt
  ...
  echo "Calling Logon parser ..."
  cd $MYPATH/tmp/$1_tmp
  sh $HOME_anu_test/Parsers/logon-parser/src/run_logon_parser.sh one_sentence_per_line.txt > /dev/null
  mv $LOGONTMP/one_sentence_per_line.txt_logon  $MYPATH/tmp/$1_tmp/
  #sed -n '1h;2,$H;${g;s/}\n/}\n;~~~~~~~~~~\n/g;p}' $MYPATH/tmp/$1_tmp/one_sentence_per_line.txt_logon | sed 's/^SKIP:/;~~~~~~~~~~\nSKIP:/g' >  $MYPATH/tmp/$1_tmp/one_sentence_per_line.txt_logon_tmp.txt
  $HOME_anu_test/Parsers/logon-parser/src/add_info_for_no_parse.out < one_sentence_per_line.txt_logon > one_sentence_per_line.txt_logon.txt
  $HOME_anu_test/Anu_src/split_file.out  one_sentence_per_line.txt_logon.txt dir_names.txt logon_output.txt
 ...
 while read line
 do
                grep -q '(No Parse Found)' $HOME_anu_tmp/tmp/$1_tmp/$line/logon_output.txt
                if [[ $? -eq 0 ]] ;
