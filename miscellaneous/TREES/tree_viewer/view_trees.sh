########################   To run all the trees #############################
 ...
 MYPATH=$HOME_anu_test/Parsers/stanford-parser/stanford-parser-full-2015-12-09
 ...
 java -mx800m -cp "$MYPATH/*:" edu.stanford.nlp.trees.EnglishGrammaticalStructure -treeFile ~/$1_derived_tree  -CCprocessed -conllx -keepPunct -lexicalize  > ~/$1_dependency_tree

 $HOME_anu_test/Parsers/stanford-parser/src/mapping-punctuations.out  < ~/$1_derived_tree >  ~/$1_derived_tree.cons

 java -mx900m -cp "$MYPATH/*:" edu.stanford.nlp.trees.TreePrint -options "lexicalize" ~/$1_derived_tree.cons > ~/$1_lexicalize_output
