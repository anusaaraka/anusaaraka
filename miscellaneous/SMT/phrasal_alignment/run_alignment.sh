...
python3 print-ids.py $1/eng_tok_org1  > $1/eng_tok_org2
python3 print-ids.py $1/hnd  > $1/hnd2

echo "Map english offsets with Word ids" >> $1/phrasal_error
python3 map-eng-offset.py $1/key_tmp.dat $1/map.dat  $1/map-wrd-id.dat >  $1/key.dat
...
python3 match_value_in_hnd.py  $1/hnd  $1/key-val.dat $1/graph_input $1/eng_tok_org $1/graph_output $1/english_left_over.dat > $1/match-value.dat 2>> $1/phrasal_error

echo "Phrase alignment" >> $1/phrasal_error
./replace-punctuation.out < $1/hnd > $1/hnd1

echo "Getting hindi offsets"  >> $1/phrasal_error
sh get_mul_pat.sh  $1/match-value.dat  $HOME_anu_test/multifast-v1.4.2/src/phrasal_mwe/extract_hindi_key.c 2>> $1/phrasal_error
...
python3 $MYPATH/map-hindi-offset.py  $1/key_hindi_phrase.dat $1/map_hindi_offsets.dat $1/match-value.dat > $1/match-value-with-hindi-wrdid.dat 2>> $1/phrasal_error
...
if [ "'$var'" == "'NO PATH'" ] ; then
	...
	python3 print_shortest_path.py $1/match-value-with-hindi-wrdid.dat  $1/graph_output > $1/shortest-path-value.dat 2>> $1/phrasal_error
	python3 get_phrase.py  $1/shortest-path-value.dat  $1/hnd2 $1/left-over-words1.dat  $1/left-over-words.dat> $1/align_eng.dat 2>> $1/phrasal_error
	python3 map_eng_offset-to_wrdids.py  $1/align_eng.dat  $1/map-wrd-id.dat > $1/mapped.dat 2>> $1/phrasal_error
	python3  map-wrd-offset-anu-ids.py $1/mapped.dat  > $1/mapped-1.dat  2>> $1/phrasal_error
	echo "Word alignment" >> $1/phrasal_error
	python3 get_word_align.py Word-to-Word-dict_$2.txt   $1/mapped.dat $1/hnd2 $1/left $1/left1 > $1/wrd-to-wrd.txt1 2>> $1/phrasal_error
	echo "Aligning left over words" >> $1/phrasal_error  2>> $1/phrasal_error
#	python3 align-left-over-wrds.py  $1/wrd-to-wrd.txt1  $1/hnd  $1/left $1/left-over-words.dat $1/align_left_over_wrds.dat > $1/wrd-to-wrd.dat 2>> $1/phrasal_error
	python3 mapping.py $1/wrd-to-wrd.txt1 > $1/mapped1.dat 2>> $1/phrasal_error
