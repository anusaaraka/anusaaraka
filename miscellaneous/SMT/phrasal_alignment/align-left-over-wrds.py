...
vib = ['baje', 'Bara_meM', 'iXara-uXara', 'kA', 'ke', 'ke_anxara', 'ke_bAhara', 'ke_bAre_meM', 'ke_bAxa', 'ke_bIca', 'ke_bIca_meM', 'ke_binA', 'ke_cAroM_ora', 'ke_jEsA', 'ke_kAraNa', 'ke_liye', 'ke_lie', 'ke_nAma_se', 'ke_nIce', 'ke_nikata', 'ke_pakRa_meM ', 'ke_pAsa', 'ke_pICe', 'ke_prawi_prema_ke_kAraNa',
       'ke_samparka_meM', 'ke_sAWa', 'ke_Upara', 'ke_usa_pAra', 'ke_viRaya_meM', 'ke_viruxXa', 'ke_xOrAna', 'ke_xvArA', 'kI', 'kI_apekRA', 'kI_ora', 'kI_waraha', 'ko', 'lagaBaga', 'meM', 'meM_se', 'mqwyu_uparAnwa', 'ne', 'para', 'se', 'se_lagA_huA', 'se_pahale', 'vAlA', 'waka', 'xvArA', 'ke_sahiwa']
...
# to use del_item function 0th sentence is checked outside.
            ...
            if phrase_left_over_lst[j] == item:
                if phrase_left_over_lst[j] in verb_list:
                    ...
                    if hi_s_l[index-1] == val[1]:
                        v = val[0] + '\t' + val[1] + \
                            '_' + phrase_left_over_lst[j]
                        align_dic[int(key)] = v
                        new_lst.append(phrase_left_over_lst[j])
                        replace_item(
                            phrase_left_over_lst[j], phrase_left_over_lst)
...
#						print '$$$', key, v
                    ...
                    if hi_s_l[index] == left_lst[i]:
                        # Comparing with previous word in hindi sentence
                        if hi_s_l[index-1] == val[1]:
                            if val[1] == '-':
                                tmp_v = align_dic[int(key-1)].split('\t')
                                # Comparing with two words previous in hindi sentence
                                if hi_s_l[index-2] == tmp_v[1]:
...
#								print '***', val
    ...
    print('(anu_id-anu_mng-man_mng' + '\t' +
          str(key) + '\t' + align_dic[key] + ')')
al_left_file.write(
    '@After @applying @huristics:\n@phrase_@left_@over_@words:::\t')
