...
def check_key_in_dic(char):
    lst = []
    for key in list(sound_dic.keys()):
    ...
    for k in range(0, len(s_lst)):
        if s_lst[k] == hnd[h_index:h_index+len(s_lst[k])]:
            if k == len(s_lst)-1:
                ...
                stack.append(
                    eng + ' ' + s_lst[k] + ' ' + str(e_index) + ' ' + str(h_index))
            else:
                j = h_index + len(s_lst[k])
                stack.append(
                    eng + ' ' + s_lst[k] + ' ' + str(e_index) + ' ' + str(j))
        if s_lst[k] == '-':  # To handle silent words
            stack.append(eng + ' ' + s_lst[k] + ' ' +
                         str(e_index) + ' ' + str(h_index))
...
#						print '**Current hnd is ', str2
        ...
        if count > max_eng_len+10 and stack == []:  # To stop while loop assuming maximum index+10 len
            break
        o = ''.join(str1) + ' ' + ''.join(str2) + ' ' + \
            ''.join(partial_eng_match) + ' ' + ''.join(partial_hnd_match)
