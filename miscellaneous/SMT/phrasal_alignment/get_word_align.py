...
for line in open(sys.argv[2]):
    if 'NO PATH' in line:
        ...
        flag = 0
        # (position-eng-hnd-eng_ids	1	_More_precisely_,_a_	_aXika_yaWArWawA_se_,_@a_	1 2 2 3)
        k = line.split('\t')
...
#		hi_lst.append(k[3].split('_')[1:-1])
        ...
        for keys in list(e_h_w_dict.keys()):
            if key == keys:
                # More***0 =EQ= aXika,*SUKH* precisely***1 =EQ= yaWArWawA,*SUKH* ,***2 =EQ= ,,*SUKH* a***3 =EQ= @a,*SUKH*
                val = e_h_w_dict[keys].split(',*SUKH*')
                del val[-1]
...
#	print hi_s_l
    for k in range(0, len(hi_s_l)-1, 2):
        if hi_s_l[k+1] != 'REPLACED':
            left_over.write('%s %s %s**,**' %
                            (hi_s_l[k], hi_s_l[k+1],  hi_s_l[k+2]))
            left_over1.write(
                '(word_offset-word_left_over_wrd\t%s\t%s)\n' % (hi_s_l[k], hi_s_l[k+1]))
