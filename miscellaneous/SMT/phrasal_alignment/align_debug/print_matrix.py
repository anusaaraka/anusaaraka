...
print('<th colspan="', cols, '">',
      '<style type="text/css"> div {text-align: left;}</style><div>', e_sent, '</div> </th>')
print('<th colspan="', cols, '">',
      '<style type="text/css"> div {text-align: left;}</style><div>', anu_sent, '</div> </th>')
print('<th colspan="', cols, '">',
      '<style type="text/css"> div {text-align: left;}</style><div>', hin_sent, '</div> </th>')
...
print('<th colspan="', cols, '">', '<FONT COLOR=#FF3333>',
      '<style type="text/css"> div {text-align: left;}</style><div>', man_wrds, '</div> </th>')
print('\t')
...
print('<th colspan="', cols, '">', '<FONT COLOR=#FF3333>',
      '<style type="text/css"> div {text-align: left;}</style><div>', eng_wrds, '</div> </th>')
print('\t')
...
print('\t', end=' ')
for key in list(eng_wrds.keys()):
    print('<span style="font-weight: bold;">', key, '</span>\t', end=' ')
print()
...
while (count <= int(man_length)):
    ...
    for line in file(sys.argv[1]):
        if line.startswith(str1):
            ...
            if row in list(row_col_dict.keys()):
...
lst2 = []
for r in range(rows):
    print('<b>', '<FONT COLOR=purple>', h_wrds[r], '</FONT>', '</b>\t', end=' ')
    for c in range(cols):
        if h_wrds[r] in list(row_col_dict.keys()):
            ...
            if man_wrd_ids != []:
                print('<FONT COLOR=brown>', '<b>', '/ '.join(man_wrd_ids), '</b>', '\t', end=' ')
            else:
                print('.\t', end=' ')
        else:
            print('.\t', end=' ')
...
if no_match_list != []:
    print('<th colspan="', cols, '">', '<style type="text/css"> div {text-align: left;}</style><div>',
          '<FONT COLOR=red>', 'no_match_found :', ' / '.join(no_match_list), '</FONT>', '</div> </th>')
...
for c in range(1, cols+1):
    print('<FONT COLOR=blue>', '<b>', tree_info_dict[c], '</b>', '\t', end=' ')
...
for c in range(1, cols+1):
    ...
    if count > 0:
        t1 = tree1[:-rcount]
#		print t1, "---------"
        print('<FONT COLOR=blue>', '<b>', t1[rcount:], '</b>', '\t', end=' ')
#		print '<FONT COLOR=blue>','<b>',t1[rcount+1:],'</b>', '\t',
    else:
        #		t1=tree1[lcount+1:]
        t1 = tree1[lcount:]
#		print t1, "---------"
        print('<FONT COLOR=blue>', '<b>', t1[:-lcount], '</b>', '\t', end=' ')
