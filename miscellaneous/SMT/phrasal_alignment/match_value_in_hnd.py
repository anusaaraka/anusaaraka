...
for line in open(sys.argv[2]):  # argv[2] is dictionary
    ...
    # _The_loss_of_electrons_from_the_n-region_       _-_kRewra_se_ilektroYnoM_kI_hAni_/_@n_-_kRewra_se_ilektroYnoM_kI_hAni_  1       8
    lst = line.split('\t')
    if '/' in lst[1]:
        hnd_mng = lst[1].split('/')
        for i in hnd_mng:
...
for n, nbrs in MG.adjacency_iter():
    for nbr, edict in list(nbrs.items()):
        minvalue = min([d['weight'] for d in list(edict.values())])
...
english_left.write('A. English_left_over_words  :\t')
