...
path = sys.argv[2]  
c_file = open(path + 'count_dict.txt', 'w')
...
#				v = str(dic[key]) + '\t' +  key
				...
				v = new_dic1[len(key)] + '\t' +  str(dic[key]) + '\t' +  key
				new_dic1[len(key)] =  v
#		for each in reversed(sorted(new_dic1)):
		for each in sorted(new_dic1):
			c_file.write('%s\t%s\n' % (each, new_dic1[each]))
		c_file.write(line)
...
c_dic = open(path + 'count_dict.txt', 'r')
