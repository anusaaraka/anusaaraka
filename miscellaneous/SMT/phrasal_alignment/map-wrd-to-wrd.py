...
for line in open(sys.argv[1]):
    # Ex: (position-eng-hnd-eng_ids	1	_More_precisely_,_a_	_aXika_yaWArWawA_se_,_@a_	1 2 2 3)
    lst = line.split('\t')
    if ';~~~~~~~~~~\n' in line:
        print(line.strip())
        ...
        for item in df:
            # _More_precisely_,_a_	_aXika_yaWArWawA_,_@a_	_aXika_yaWArWawA_se_,_@a_
            each = item.split('\t')
            ids = lst[4][:-2].split()
                ...
                if len(key) == len(hnd_key) and len(key) == len(group_lst):
                    ...
                    for i in range(0, len(key)):
                        if hnd_key[i] == '#####':
                            ...
                            print('(anu_id-anu_mng-man_mng ' +
                                  ids[i], key[i], hnd_key[i] + ')')
                # Ex: to add 'se' to the previous word
                elif len(key) == len(hnd_key) and len(key) != len(group_lst):
                    for k in hnd_key:
                        if k in group_lst:
                    ...
                    for i in range(0, len(key)):
                        if hnd_key[i+count] == '#####':
                            ...
                            print(
                                '(anu_id-anu_mng-man_mng ' + ids[i], key[i], hnd_key[i+count] + '_' + hnd_key[i+1+count] + ')')
                            count = count+1
                        else:
                            print('(anu_id-anu_mng-man_mng ' +
                                  ids[i], key[i], hnd_key[i+count] + ')')
