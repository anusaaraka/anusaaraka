        ...
        for key in sorted(map_dic):
            ...
            if key in dic:
                if int(val[6]) in dic:  # maksvela-samIkaraNa
                    print(str(dic[key]) + '\t' + val[1] + '\t' + val[2] + '\t' + val[3] + '\t' + val[4] + '\t' +
                          val[5] + '\t' + str(dic[int(val[6])]) + '\t' + val[7] + '\t' + val[8] + '\t' + val[9])
                else:
                    print(str(dic[key]) + '\t' + val[1] + '\t' + val[2] + '\t' + val[3] + '\t' + val[4] +
        ...
        if lst[1] != '-' and nxt_lst[1] != '-':
            ...
            if prev_lst[6] == nxt_lst[0]:  # xina - rAwa
                ...
                if int(prev_lst[0]) == 1:
                    ...
                    dic[int(prev_lst[0])] = int(
                        prev_lst[0]) - count  # yaxA - kaxA
                wrd = prev_lst[1] + lst[1] + nxt_lst[1]
                root = prev_lst[2] + lst[1] + nxt_lst[2]
                prev_lst[1] = wrd
                prev_lst[2] = root
                ...
                grouped_id_lst.append(prev_lst[0])
                grouped_id_lst.append(nxt_lst[0])
                # as we decrease two ids so increamenting count again.
                count += 1
            else:
                if prev_lst[3] != 'v':
                    dic[int(prev_lst[0])] = int(prev_lst[0]) - \
                        count  # bahu - sAMskqwika
                    wrd = prev_lst[1] + lst[1] + nxt_lst[1]
                    root = prev_lst[2] + lst[1] + nxt_lst[2]
                    prev_lst[1] = wrd
                    prev_lst[2] = root
