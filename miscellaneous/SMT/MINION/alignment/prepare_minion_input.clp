...
;------------------------------------------------------------------------------------------------------------
;Added by Shirisha Manju (12-01-13)
(defrule print_head_cost_info
(declare (salience 896))
(grp_hids-slot_ids-var_ids $?grp)
(not (got_head_cost_info))
=>
	(printout ?*minion_fp* " #The constraint ineq(x, y, k) ensures that x <= y + k "  crlf crlf)
	(assert (got_head_cost_info))
)
;------------------------------------------------------------------------------------------------------------
;Added by Shirisha Manju (20-08-12)
