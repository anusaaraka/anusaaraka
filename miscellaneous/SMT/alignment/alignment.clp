...
=>
        (retract ?f0 ?f ?f1)
        (assert (anu_id-manual_id-a_grp_mng-m_grp_mng  ?a_id ?m_id $?grp_mng - $?grp_mng))
...
=>
	(retract ?f0 ?f ?f1)
	(assert (anu_id-manual_id-a_grp_mng-m_grp_mng  ?a_id ?m_id $?w1 - $?w))
        (assert (manual_hin_sen $?pre $?post))
...
=>
	(retract ?f0 ?f ?f1)
	(assert (anu_id-manual_id-a_grp_mng-m_grp_mng  ?a_id ?m_id $?w1 - $?word))
        (assert (manual_hin_sen $?pre $?post))
...
?f2<-(manual_hin_sen $?pre $?m_word $?post)
 =>
	(retract ?f0 ?f1 ?f2)
	(assert (anu_id-manual_id-a_grp_mng-m_grp_mng   ?a_id  ?m_id $?a_word - $?m_word))
	(assert (manual_hin_sen $?pre $?post))
...
=>
	...
	(bind $?m_mng (create$ ?w $?word ?v))
        (assert (anu_id-manual_id-a_grp_mng-m_grp_mng  ?a_id $?a_mng - $?m_mng))
        (assert (manual_hin_sen $?pre $?post))
...
=>
	...
	(assert (anu_id-manual_id-a_grp_mng-m_grp_mng  ?a_id ?m_id $?a_mng - $?m_mng))
	(assert  (manual_hin_sen $?pre $?post))
	(assert (modified_sen_with_ora))
