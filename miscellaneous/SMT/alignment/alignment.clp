...
=>
        (retract ?f0 ?f ?f1)
        (assert (anu_id-a_grp_mng-manual_id-m_grp_mng  ?a_id  $?grp_mng ?m_id $?grp_mng))
...
=>
	(retract ?f0 ?f ?f1)
	(assert (anu_id-a_grp_mng-manual_id-m_grp_mng  ?a_id  $?w1 ?m_id $?w))
        (assert (manual_hin_sen $?pre $?post))
...
=>
	(retract ?f0 ?f ?f1)
	(assert (anu_id-a_grp_mng-manual_id-m_grp_mng  ?a_id  $?w1 ?m_id $?word))
        (assert (manual_hin_sen $?pre $?post))
...
?f2<-(manual_hin_sen $?pre $?m_word $?post)
 =>
	(retract ?f0 ?f1 ?f2)
	(assert (anu_id-a_grp_mng-manual_id-m_grp_mng  ?a_id  $?a_word ?m_id $?m_word))
	(assert (manual_hin_sen $?pre $?post))
...
=>
	...
	(assert (anu_id-a_grp_mng-manual_id-m_grp_mng  ?a_id  $?a_mng ?m_id $?m_mng))
        (assert (manual_hin_sen $?pre $?post))
