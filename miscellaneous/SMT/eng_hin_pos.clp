...
;----------------------------------------------------------------------------------------------------------------------------
 (defrule rm_repeated_lwg_id
 (declare (salience 2001))
 ?f<-(root-verbchunk-tam-chunkids ?root ?vc ?tam $?ids ?id)
 (not (root-verbchunk-tam-chunkids ? ? ? $?ids ?id1&:(> ?id ?id1)))
 ?f1<-(root-verbchunk-tam-chunkids ?root1 ?vc1 ?tam1 $?ids ?id2)
 (test (neq ?id ?id2))
 (test (> (length $?ids) 0))
  =>
 	(retract ?f1)
 ...
 ?f1<-(pada_info (group_head_id ?g_h)(group_ids $?grp_ids)(preposition $?prep_ids))
 ?f2<-(pada_info (group_head_id ?g_h1)(group_ids $?grp_ids1)(preposition $?prep_ids1))
 (test (neq ?g_h ?g_h1))
 (test (or (neq (member$ ?id $?grp_ids) FALSE) (neq (member$ ?id $?prep_ids) FALSE)))
 (test (or (neq (member$ ?id1 $?grp_ids1) FALSE)(neq (member$ ?id1 $?prep_ids1) FALSE)))
 (not (id-checked_for_combine_padas ?g_h))
 ?f3<-(hindi_id_order  $?hin_order)
  =>
 	(retract ?f ?f2 ?f3)
	(bind $?grp (create$ (sort_grp $?g1 ?id $?g2 ?id1 $?g3 $?grp_ids $?grp_ids1 $?prep_ids $?prep_ids1)))
        (bind $?grp (create$ (delete-member$ $?grp 0)))
        (modify ?f1 (group_head_id ?g_h)(group_ids $?grp)(preposition 0))

        (assert (id-HM-source-grp_ids ?g_h - - $?grp))
        (assert (id-checked_for_combine_padas ?g_h))

         (loop-for-count (?i 1 (length $?hin_order))
                  (bind ?j (nth$ ?i $?hin_order))
                  (if (eq ?j ?g_h1) then
                      (bind $?hin_order (delete-member$ $?hin_order ?g_h1))
                  ))
 ...
 (defrule delete_prep_id_from_affected_ids
 (declare (salience 2001))
 ?f<-(id-HM-source-grp_ids   ?h_id  ?hmng  ?src $?pre ?id $?pos)
 (test (or (neq (length $?pre) 0) (neq (length $?pos) 0)))
 (not (id-checked-for-prep ?id))
 (pada_info (group_cat PP)(preposition $?prep_ids))
 ;(test (neq (member$ ?id $?prep_ids) FLASE))
 (test (member$ ?id $?prep_ids))
; (test (neq (implode$ $?prep_ids) "0"))
  =>
  (retract ?f)
 ; (loop-for-count (?i 1 (length $?ids))
 ;                 (bind ?j (nth$ ?i $?ids))
 ;                 (if (member$ ?j $?prep_ids) then
 ;                     (bind $?ids (delete-member$ $?ids ?j))
 ;                     (printout t $?ids crlf)
 ;                 ))
 (assert (id-checked-for-prep ?id))
 (assert (id-HM-source-grp_ids   ?h_id  ?hmng  ?src $?pre $?pos))
 )
...
; (expr  $?pr ?id $?po)
; (hindi_id_order $?prev ?id $?post)
; (id-HM-source-grp_ids   ?  ?  ? $?pre ?id $?pos)
; (not (id-pos ?id))
; =>
;                 (bind $?hin_ord (create$ $?prev ?id $?post))
;                 (bind ?pos (member$ ?id $?hin_ord))
;                 (assert (id-pos ?id))
;                 (bind $?grp_ids (sort_grp $?pre ?id $?pos))
;                 (assert(hin_pos-src-eng_ids ?pos - $?grp_ids))
; )
;---------------------------------------------------------------------------------------------------------------------------
 ;Print to a file 
